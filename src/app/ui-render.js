(function initUiRender(global) {
  ...
  function showJob() {
    ...
    const routeRows = routes.map((route) => {
      ...
      return `<tr><td>${route.name}</td><td>${route.intro || "-"}</td><td>等级${requiredLevel}+ / 基础银两${route.baseSilver || 0}</td><td>${isSelected ? "<span style='color:#2f855a;'>当前</span>" : `<button class='small-btn' ${canSelect ? "" : "disabled"} onclick="setTradeRoute('${route.id}')">选择</button>`}</td></tr>`;
    }).join("");

    setMainTitle("江湖职业");
    setMainContent(`${renderNoticeHtml()}<div class="notice">当前职业：<b>${player.job}</b></div><div class='card'><h3>跑商路线</h3><div class='list-line'>当前路线：<b>${currentRoute?.name || "暂无"}</b>（ID：${currentRoute?.id || "-"}）</div><div class='table-box'><table><thead><tr><th>路线</th><th>说明</th><th>要求/基础收益</th><th>操作</th></tr></thead><tbody>${routeRows || "<tr><td colspan='4'>暂无跑商路线配置</td></tr>"}</tbody></table></div></div><div class="table-box"><table><thead><tr><th>职业</th><th>说明</th><th>产出</th><th>选择</th></tr></thead><tbody>${rows}</tbody></table></div><div class="shop-actions"><button class="action-btn" onclick="doJob()">执行职业动作</button><button class="action-btn" onclick="work()">执行一次跑商（体力换银两）</button></div><div class='card'><h3>跑商最近记录</h3>${renderRecentLogbook("economy", 4, "暂无跑商记录。")}<div class='shop-actions'><button class='small-btn' onclick="showLogbook('economy')">查看完整跑商日志</button></div></div>`);
  }
