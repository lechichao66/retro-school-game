(function initUiRender(global) {
  ...
  function updateTopBar() {
    ...
    document.getElementById("exp").textContent = `${panel.exp} / ${window.__JH_LEVEL_SYSTEM__?.getRequiredExpForLevel?.(panel.level) || panel.level * 100}`;
    ...
    setMainContent(`
      ...
      <div class="status-grid status-grid-status">
        <div class="card">
          ...
          <div class="list-line">经验：${player.exp} / ${window.__JH_LEVEL_SYSTEM__?.getRequiredExpForLevel?.(player.level) || player.level * 100}</div>
  ...
  function showSect() {
    ...
    const martialRows = Object.keys(grouped).map((cat) => `<div class='card'><h3>${getMartialCategoryLabel(cat)}</h3>${grouped[cat].map((skill) => {
      ...
      return `<div class='list-line'><b>${skill.name}</b>【${skill.grade || "不入流"}】</div><div class='list-line'>学习条件：等级${skill.learnReq?.level || 1}、贡献门槛${skill.learnReq?.contribution || 0}、银两${skill.learnReq?.money || 0}</div><div class='list-line'>武功等级：${skillLevel}（上限：人物等级+10）</div><div class='list-line'>研习消耗：银两${reducedMoney} + 经验${nextExp}</div><div class='list-line'>效果：${skill.effect}</div><div class='shop-actions'>${learned ? "<button class='small-btn' disabled>已学会</button>" : `<button class='small-btn' onclick="learnMartialSkill('${skill.id}')">学习</button>`}<button class='small-btn' onclick="trainMartialSkill('${skill.id}','resource')">研习升级</button></div><hr>`;
    }).join("")}</div>`).join("");
  ...
  function showInn(section) {
    ...
    const lastRunSummary = lastRun
      ? `<div class='card'><h3>最近一轮打图摘要</h3><div class='list-line'>时间：${lastRun.time || "未知"}｜宝图：${lastRun.mapName || "未知"}｜地点：${lastRun.area || "未知"}</div><div class='list-line'>轮次：${lastRun.attempts || 0}次｜扑空${eventCounter.empty || 0}｜银两点${eventCounter.findMoney || 0}｜物资点${eventCounter.findItem || 0}｜机关${eventCounter.trap || 0}｜遭遇${eventCounter.ambush || 0}</div><div class='list-line'>奖励：银两${lastRun.rewards?.money || 0}｜物品：${Object.keys(lastRun.rewards?.items || {}).length ? Object.keys(lastRun.rewards.items).map((name) => `${name} x${lastRun.rewards.items[name]}`).join("，") : "无"}</div><div class='list-line'>损耗：气血 -${lastRun.hpLoss || 0}</div></div>`
      : "<div class='card'><h3>最近一轮打图摘要</h3><div class='list-line'>暂无打图摘要，请先启用宝图并执行一轮打图。</div></div>";
    const treasureModule = `<div class='card'><h3>宝图模块</h3><div class='list-line'>主玩法：获得宝图 → 使用宝图 → 一轮打图（10次）→ 结算奖励与摘要。</div><div class='list-line'>兼容保留：仍可使用旧“挖宝”单次结算路径。</div><div class='shop-actions'><button class='small-btn' onclick='grantTreasureMap()'>获得一张样板宝图</button><button class='small-btn' onclick='runTreasureRound()'>执行一轮打图（10次）</button><button class='small-btn' onclick=\"showInn('treasure')\">刷新宝图页</button><button class='small-btn' onclick=\"showLogbook('treasure')\">查看完整宝图日志</button></div></div>${lastRunSummary}<div class='card'><h3>宝图最近记录</h3>${renderRecentLogbook("treasure", 8, "暂无宝图记录。")}</div>${treasureRows || "<div class='card'>暂无宝图，可先领取样板。</div>"}`;
